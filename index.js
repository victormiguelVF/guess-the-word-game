...
function startNewGame() {
    alert("Novo Jogo! Adivinhe a palavra :)");
    //Hide hint element
    ...
    word = ranWord.word;
    //If word chars >= 5 then max guesses = 8 else guess = 6
...
function handleInput(e) {
    ...
    //Update remain guess and checks for win lose conditions
    guessLeft.innerText = maxGuesses;
    if(correctLetters.length === word.length) {
        alert(`Parabéns! Você acertou a palavra ${word.toUpperCase()}`);
        startNewGame();
    } else if(maxGuesses < 1) {
        alert("Gmae Over! Você não tem mais tentativas!");
        for (let i = 0; i < word.length; i++) {
